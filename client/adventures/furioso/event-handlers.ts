...
export var event_handlers = {
  start: function () {
    ...
    // lantern starts out lit
    game.artifacts.get(1).is_lit = true;
    ...
    game.history.write(
      "You wake up on a hard wooden floor, somewhere dark and stuffy. You seem to have sustained a few bruises. " +
        "In the distance, you can hear shouting and ocean sounds.",
    );
    // naked message
    game.effects.print(7);
    game.history.write(
      "You hear a man's voice in the darkness. He says, 'Good, you're awake. I'm Orlando. I was kidnapped by " +
        "the smugglers, too. We need to get out of here, pronto.'",
    );
  },

  // FIXME: should be able to read dagger
    ...
    switch (exit.room_to) {
      ...
      case -11:
        ...
        if (game.monsters.get(1).isHere()) {
          game.history.write(
            "Orlando says, \"I've seen that ship before. I think they're slavers. Let's try a different one.\"",
          );
          return false;
        } else {
  ...
  afterDeath: function (monster: Monster) {
    // tentacles/squid
    console.log("monster died", monster);
    // 14 is the group, 14.0001 are the individuals. When the group dies, the whole squid is dead.
    if (monster.id === 14) {
      game.history.write("The squid is dead!");
        ...
        switch (roll) {
          ...
          case 2: {
            ...
            for (const a of game.player.inventory) {
              if (a.value > 0 && !game.data["artifacts increased"].includes(a.id)) {
                a.value += 10;
      ...
      if (roll >= 7) {
        ...
        if (favorite_item !== null) {
          if (game.data["malagur steal"] < 3) {
            ...
            game.history.write(
              "You catch " + malagur.name + " trying to steal your " + favorite_item.name + "!",
              "warning",
            );
  ...
  power: function (roll) {
    ...
    if (game.rooms.current_room.is_dark && !game.artifacts.isLightSource() && !ball.isHere()) {
      ...
      if (roll === 1 && game.player.damage > 0) {
        ...
        roll === 3 &&
        game.player.room_id < 15 &&
        game.data["water level"] > 20 &&
        game.data["water dropped"] < 4
      ) {
        game.data["water level"] -= 20;
