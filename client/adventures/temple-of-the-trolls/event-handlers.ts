...
export var event_handlers = {
  start: function () {
    game.data = {
      "troll words": ["jolicks verploeg", "vanch cur fros", "wilk nedthix", "kentry drathran", "hulmfin radkryne"],
      "active words": "",
      "holfane speaks": 0,
    };
  ...
  give: function (arg: string, artifact: Artifact, recipient: Monster) {
    ...
    if (recipient.id === 2) {
      if (artifact.id === 13 || artifact.id === 14) {
        game.history.write(
          'Grommick looks at your offering and shakes his head. "I\'ll need a greater reward than that."',
        );
        return false;
  ...
  say: function (phrase) {
    ...
    switch (phrase) {
      case "trezore":
        ...
        break;
      case "qorgaw":
        if (!game.player.timed_effects.qorgaw) {
          game.effects.print(17);
          const duration = 5 + game.diceRoll(1, 12);
          // game.player.spell_counters['qorgaw'] = game.diceRoll(1, 12) + 5;
          game.player.timed_effects.qorgaw = {
            name: "qorgaw",
            duration,
            timer: duration,
            counters: 1,
  ...
  spellExpires: function (spell_name: string) {
