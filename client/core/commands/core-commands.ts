        ...
        if (art && art.container_id !== null) {
          ...
          if (container.monster_id === Monster.PLAYER) {
            throw new CommandException(
              `You're already carrying it. But you could REMOVE it from the ${container.name}.`,
            );
...
export class PutCommand implements BaseCommand {
  ...
  examples: string[] = ["PUT SWORD INTO SCABBARD"];
  run(verb: string, arg: string) {
...
export class DropCommand implements BaseCommand {
  ...
  examples: string[] = ["DROP SWORD", "DROP ALL - Tries to drop everything you are carrying."];
  run(verb: string, arg: string) {
    arg = arg.toLowerCase();
    ...
    if (game.triggerEvent("wear", arg, artifact)) {
      if (artifact) {
        if (artifact.type === Artifact.TYPE_WEARABLE) {
          ...
          if (
            artifact.armor_type === Artifact.ARMOR_TYPE_SHIELD &&
            game.player.weapon &&
            game.player.weapon.hands === 2
          ) {
            throw new CommandException(
              "You are using a two-handed weapon. You can only use a shield with a one-handed weapon.",
            );
          }
    ...
    if (a !== null) {
      if (game.triggerEvent("beforeOpen", arg, a)) {
        ...
        if (a.type === Artifact.TYPE_DISGUISED_MONSTER) {
          ...
          a.type === Artifact.TYPE_CONTAINER ||
...
export class TakeCommand implements BaseCommand {
  ...
  category = "interactive";
  description =
    "Takes an item back from a monster or NPC. However, they might not want to give it to you. This usually only works with friendly monsters.";
  examples: string[] = ["REQUEST SWORD FROM EDDIE", "TAKE SWORD FROM EDDIE"];
  run(verb: string, arg: string) {
    const regex_result = /(.+) from (.*)/.exec(arg);
    if (regex_result === null) {
...
export class FreeCommand implements BaseCommand {
  ...
  category = "interactive";
  description =
    "Frees a bound monster. Note that a nearby monster may be guarding the bound monster and may prevent this.";
  examples: string[] = ["FREE PRISONER"];
  run(verb: string, arg: string) {
    let message = "";
...
export class BlastCommand implements BaseCommand {
  ...
  category = "magic";
  description =
    "Casts the BLAST spell. This is a standard magic missile type spell that can damage monsters and some artifacts like doors and chests.";
  examples: string[] = [
