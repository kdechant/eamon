...
const Status: React.FC<StatusProps> = (props) => {
  ...
  const armor = game.player.inventory
    .filter((a) => a.is_worn && a.isArmor())
    .sort((a, b) => {
  ...
  return (
    <div className={`status ${statusClass} d-md-block col-md-5`}>
      <div className="status-widget player-stats">
        <div className="container">
          <div className="row">
            ...
            <div className="hp col-4 text-right">
          ...
          <div className="weapon-abilities row no-gutters">
            <div className="axe col">
              Axe:
              <br />
              {game.player.weapon_abilities[1]}%
            </div>
            <div className="bow col">
              Bow:
              <br />
              {game.player.weapon_abilities[2]}%
            </div>
            <div className="club col">
              Club:
              <br />
              {game.player.weapon_abilities[3]}%
            </div>
            <div className="spear col">
              Spear:
              <br />
              {game.player.weapon_abilities[4]}%
            </div>
            <div className="sword col">
              Sword:
              <br />
          ...
          <div className="spell-abilities row no-gutters">
            <div className="col">
              Blast:
              <br />
              {game.player.spell_abilities.blast}%
            </div>
            <div className="col">
              Heal:
              <br />
              {game.player.spell_abilities.heal}%
            </div>
            <div className="col">
              Power:
              <br />
              {game.player.spell_abilities.power}%
            </div>
            <div className="col">
              Speed:
              <br />
          ...
          {game.player.weapon ? (
            <div className="weapon row">
              <div className="col">
                Ready weapon: {titleCase(game.player.weapon.name)} ({game.player.weapon.dice}d{game.player.weapon.sides}
                )
              </div>
            </div>
          ) : (
            <div className="weapon none row">
      ...
      <div className="status-widget monsters">
        ...
        {!inTheDark && (
          <div className="monsters-list">
            ...
            {game.monsters.visible.length === 0 && <span className="monster none">no one</span>}
          </div>
        )}
        ...
        {inTheDark && (
          <div className="monsters-list">
            ...
            {game.monsters.visible.length === 0 && <span className="monster none">You think you're alone.</span>}
          </div>
        )}
      </div>
      ...
      <div className="status-widget inventory">
        ...
        <p className="weight">
          Weight carried: {game.player.weight_carried}/{game.player.hardiness * 10}
        </p>
      </div>
    </div>
  );
...
const StatusMonster: React.FC<StatusMonsterProps> = (props) => {
  ...
  if (monster.reaction === Monster.RX_HOSTILE) {
    className += "hostile";
  }
  ...
  return (
    <div className={className}>
      {singular ? (
        ...
        <span>
          {visible_children.length} {monster.name_plural}
        </span>
      )}{" "}
      - {monster.reaction}
    </div>
      ...
      {artifact.is_open && (
        <div>
          {artifact.contents.map((item) => (
            <div className="artifact-contents" key={item.id}>
              {item.getDisplayName()}
            </div>
          ))}
        </div>
