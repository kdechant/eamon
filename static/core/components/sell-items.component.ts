...
@Component({
  ...
  template: `
    <div *ngIf="game.player.weapons.length > 4">
      <p>As you enter the Main Hall, Lord William Missilefire approaches you and says, "You have too many weapons to
        keep them all. Four is the legal limit."</p>
      ...
      <div *ngIf="game?.player?.inventory">
        <table class="table artifacts-list">
          ...
          <tr class="artifact" *ngIf="artifact.type == 2 || artifact.type == 3">
            ...
            <td>
              <button class="btn" (click)="sell(artifact)">Sell</button>
            </td>
          </tr>
          </tbody>
        </table>
    ...
    <div *ngIf="game.player.weapons.length <= 4">
      ...
      <p *ngFor="let msg of game.after_sell_messages">{{msg}}</p>
